...
export const IGNORED_DIRS = new Set([".git", ".venv", ".idea", "__pycache__", "node_modules"]);
...
export function failure<T extends Record<string, unknown> = Record<string, unknown>>(
  error: unknown,
  data = {} as T,
): ToolResult<T> {
  return { ok: false, data, error: String(error instanceof Error ? error.message : error) };
}

export async function workspacePath(value: string): Promise<[string, string]> {
  if (typeof value !== "string" || value.length === 0) throw new Error("路径必须是非空字符串");
  ...
  if (!isInside(root, await realpath(existing))) throw new Error("路径不能超出工作区");
