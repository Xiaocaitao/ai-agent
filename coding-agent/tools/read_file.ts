...
export async function readFileTool(path: string, startLine = 1, maxLines = 200) {
  if (!Number.isInteger(startLine) || startLine < 1) return failure("start_line 必须是正整数");
  if (!Number.isInteger(maxLines) || maxLines < 1) return failure("max_lines 必须是正整数");
  try {
    const [target, relativePath] = await workspacePath(path);
