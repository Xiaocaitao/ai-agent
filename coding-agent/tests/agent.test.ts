...
import { ReActAgent, resolveWorkspace, sanitizeUnicode } from "../agent.ts";
...
test("ReActAgent 无工具调用时返回最终回答", async () => {
  ...
  const agent = new ReActAgent(client, "model-x", "prompt", new ToolRegistry([], {}), 3);
...
test("ReActAgent 执行工具并记录 Observation", async () => {
  ...
  const output: string[] = [];
  const agent = new ReActAgent(client, "model-x", "prompt", new ToolRegistry(echoSpecs, { echo: ({ text }) => ({ ok: true, data: { text }, error: null }) }), 3);
...
test("ReActAgent 将工具错误转为 Observation", async () => {
  ...
  for (const item of cases) {
    ...
    const agent = new ReActAgent(client, "model-x", "prompt", new ToolRegistry(specs, item.handlers), 3);
...
test("ReActAgent 达到最大步骤时停止", async () => {
  ...
  const agent = new ReActAgent(client, "model-x", "prompt", new ToolRegistry(echoSpecs, { echo: ({ text }) => text }), 1);
